...
- Once again, due to `position: fixed`, there are some caveats with using modals on mobile devices. [See our browser support docs]({{ .Site.BaseURL }}/docs/{{ .Site.Params.docs_version }}/getting-started/browsers-devices/#modals-and-dropdowns-on-mobile) for details.
...
[Tooltips]({{ .Site.BaseURL }}/docs/{{ .Site.Params.docs_version }}/components/tooltips/) and [popovers]({{ .Site.BaseURL }}/docs/{{ .Site.Params.docs_version }}/components/popovers/) can be placed within modals as needed. When modals are closed, any tooltips and popovers within are also automatically dismissed.
